...
func BufferReadMessage(r *bytes.Buffer) (*Message, error) {
	if r.Len() == 0 {
		return nil, lenErr
	}
	switch r.Bytes()[0] {
	case bulkStrPrefix:
		return bufferReadBulkStr(r)
	case arrayPrefix:
		return bufferReadArray(r)
	default:
		return nil, badType
	}
}
...
func bufferReadBulkStr(r *bytes.Buffer) (*Message, error) {
	b, err := r.ReadBytes(delimEnd)
	if err != nil {
		return nil, err
	}
...
func bufferReadArray(r *bytes.Buffer) (*Message, error) {
	b, err := r.ReadBytes(delimEnd)
	if err != nil {
		return nil, err
	}
